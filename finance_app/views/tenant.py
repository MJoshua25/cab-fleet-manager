...
def add_insurance(request: HttpRequest, tenant: str) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        insurance.save()
        messages.success(request, "Ajout d'une assurance éffectuée")
        return redirect('core:tenant:finance:insurance_list', tenant=tenant.unique_domain)
    else:
        messages.error(request, "Ajout d'une assurance non éffectuée")
...
def update_insurance(request: HttpRequest, tenant: str, type_id: int) -> HttpResponse:
    ...
    insurance.save()
    messages.success(request, "Modification d'assurance éffectuée")
    return redirect('core:tenant:finance:insurance_list', tenant=tenant.unique_domain)
...
def delete_insurance(request: HttpRequest, tenant: str, type_id: int) -> HttpResponse:
    ...
    contract.delete()
    messages.success(request, "Suppression d'assurance éffectuée")
    return redirect('core:tenant:finance:insurance_list', tenant=tenant.unique_domain)
...
def add_insurance_payment(request: HttpRequest, tenant: str) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        i.save()
        messages.success(request, "Ajout de paiement d'assurance éffectué")
        return redirect('core:tenant:finance:insurance_payment_list', tenant=tenant.unique_domain)
    else:
        messages.error(request, "Ajout de paiement d'assurance non éffectué")
        return redirect("core:tenant", tenant=tenant.unique_domain)
...
def update_insurance_payment(request: HttpRequest, tenant: str, type_id: int) -> HttpResponse:
    ...
    i_payment.save()
    messages.success(request, "Modification de paiement d'assurance éffectuée")
    return redirect('core:tenant:finance:insurance_payment_list', tenant=tenant.unique_domain)
...
def delete_insurance_payment(request: HttpRequest, tenant: str, type_id: int) -> HttpResponse:
    ...
    contract.delete()
    messages.success(request, "Suppression de paiement d'assurance éffectuée")
    return redirect('core:tenant:finance:insurance_payment_list', tenant=tenant.unique_domain)
...
class OutageListView(TenantAwareViewMixin, ListView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            outage = form.save()
            messages.success(request, "Ajout de panne éffectué")
...
class OutageUpdateView(TenantAwareViewMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            form.save()
            messages.success(request, "Modification de panne éffectuée")
        return redirect('core:tenant:finance:outage_list')
...
class OutageDeleteView(TenantAwareViewMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not outage:
            return redirect('core:tenant:finance:outage_list')
        outage.delete()
        messages.success(request, "Suppression de panne éffectuée")
        return redirect('core:tenant:finance:outage_list')
...
def add_oil_change(request: HttpRequest, tenant: str) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        i.save()
        messages.success(request, "Ajout de vidange éffectué")
        return redirect('core:tenant:finance:oil_change_list', tenant=tenant.unique_domain)
    else:
        messages.error(request, "Ajout de vidange non éffectué")
        return redirect("core:tenant", tenant=tenant.unique_domain)
...
def update_oil_change(request: HttpRequest, tenant: str, type_id: int) -> HttpResponse:
    ...
    oilchange.save()
    messages.success(request, "Modification de vidange éffectuée")
    return redirect('core:tenant:finance:oil_change_list', tenant=tenant.unique_domain)
...
def delete_oil_change(request: HttpRequest, tenant: str, type_id: int) -> HttpResponse:
    ...
    oilchange = fleet_models.OilChange.objects.filter(statut=True, id=type_id)[:1].get()
    oilchange.delete()
    messages.success(request, "Suppression de vidange éffectuée")
    return redirect('core:tenant:finance:oil_change_list', tenant=tenant.unique_domain)
